...
public class SplitPercentageFragment extends Fragment implements SplitFragment {
    ...
    public Map<String, Double> getSplitData(double totalAmount) {
        ...
        for (Map.Entry<String, EditText> entry : inputMap.entrySet()) {
            ...
            double pct = val.isEmpty() ? 0 : Double.parseDouble(val);
            shares.put(entry.getKey(), (pct * totalAmount) / 100.0);
        }
        return shares;
    }
    ...
    public boolean isValid(double totalAmount) {
        ...
        return Math.abs(100 - sum) < 0.1; // Ensure percentages sum to 100
    }
}